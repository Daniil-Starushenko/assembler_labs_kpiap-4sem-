...
openFiles:    
	...
	jc errorCommandLine     
	mov handler1,ax        ;помещаем идентификатор файла
	
    mov ah, 3Dh			        
	mov al, 02h	           ;для чтения и для записи
	mov cl,0h			       
	mov dx, offset file2         	      
	int 21h 
	 
	jc errorCommandLine
	mov handler2,ax        ;идентификатор
 
    xor di,di  
	lea dx,buf			   ;буфер для приема данных
