...
leftFill:
	mov	al, 004					;символ 004 для записи в видеобуфер
	mov	es:[di],al				;записываем его по адресу 0b800h:di
	inc	di						;di++
	mov	al,111				    ;цвет символа 004 соответсвует 111
...
continueRight:
    ...
    call setPiece 				;вызываем процедуру установки фигуры
...
;конвертируем фигуры в кирпичики
 blockPixel:
	...
	mul bh                      ;умножаем размер на кол-во колонок
	add di,ax                   ;добавляем смещение к адресу
	mov al,'*'					;заносим '*' в al
...
x_line     db      22h,5,5,5,5 ; столбец
y_line     db          0,1,2,3 ; строка 
x_l        db      55h,5,6,7,5
y_l        db          0,0,0,1
x_r        db      33h,5,6,7,7
y_r        db          0,0,0,1
